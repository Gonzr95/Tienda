...
import { restoreStock } from "../services/productService.js"
...
export async function updateTicket(req, res) {
  try {
    ...
    {
      // 1) Verificar si el status nuevo = cancelada
      case "cancelada":
        console.log("Es una cancelacion");
        // 2) Si el status nuevo es = cancelada y antes era otra cosa proceder 
        if(ticket.status !== "cancelada")
        {
          console.log("El estado actual del ticket no es cancelada, asi que procedo a cancelar");
          const products = await ticketService.getProductsByTicketId(id)
          const result = await restoreStock(products);
          
          

          // 3) Traer los productos y cantidades que forman parte del pedido


        }
      break;
      default :
        console.log("El estado previo del ticket era otra cancelado u otra cosa")
    }    



    // 4) uno por uno reponer las cantidades
    // 5) Actualizar 
