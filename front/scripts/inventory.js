import { devBackendURL, devImgsBaseURL, prodBackendUrl, prodImgsBaseURL } from './config.js';
...
async function loadCategories() {
    ...
    try {
        ...
        const response = await fetch(`${prodBackendUrl}/products/categories`);
...
async function loadProducts(category) {
    ...
    try {
        ...
        const response = await fetch(`${prodBackendUrl}/products?category=${encodeURIComponent(category)}`);
...
function createProductCard(product) {
    ...
    img.src = `${prodImgsBaseURL}${product.images[0]}`|| 'https://via.placeholder.com/300'; // Fallback si no hay imagen
